...
local options = {
  ...
  args = {
	...
	sellilvlgreens = {
		type = 'input',
		order = 4,
		name = 'Sell green items',
		desc = 'Sell green armor and weapons that are below or equal to this itemlevel threshold',
		set = function( info, val ) AV.db.profile.sellilvlgreens = val; end,
		get = function( info) return AV.db.profile.sellilvlgreens end,		
	},
    nonoptimal = {
    	type = 'toggle',
    	order = 5,
...
function AV:ShouldSell(link)
	...
	local ilvl = tonumber( self.db.profile.sellilvlgreens )
	if self.db.profile.sellilvlgreens and ilvl > 0 and ilvl >= itemLevel then
		
		if itemQuality == 2 and ( itemType == 'Armor' or itemType == 'Weapon' ) then 
			--self:Print( "Selling green: " .. link );
			return true
		end
...
AV_UNUSABLE_ITEMS = {
	...
	['HUNTER'] = {
		...
		['Weapon'] = { 'One-Handed Maces', 'Two-Handed Maces', 'Wands', 'One-Handed Axes', 'Two-Handed Axes', 'Daggers', 'Staves', 'Polearms', 'One-Handed Swords', 'Two-Handed Swords', 'Fist Weapons', 'Thrown' },
	},
	['MAGE'] = {
